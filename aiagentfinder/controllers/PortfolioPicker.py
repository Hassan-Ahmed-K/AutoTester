...
class PortfolioPickerController:
    def __init__(self,ui):
        ...
        self.ui.csv_dir.textChanged.connect(self.on_csv_input_changed)
        self.ui.html_dir.textChanged.connect(self.on_htm_input_changed)
        self.ui.set_dir.textChanged.connect(self.on_set_input_changed)
        self.ui.csv_browser.clicked.connect(lambda: self.browse_folder(self.ui.csv_dir))
        self.ui.html_browser.clicked.connect(lambda: self.browse_folder(self.ui.html_dir))
        self.ui.set_browser.clicked.connect(lambda: self.browse_folder(self.ui.set_dir))
        self.ui.build_btn.clicked.connect(self.on_build_portfolio_button_clicked)
        self.ui.analyze_btn.clicked.connect(self.on_build_portfolio_button_clicked)
        self.ui.show_graph_btn.clicked.connect(self.on_show_graph_clicked)
    ...
    def refresh_selected_set_files(self):
        ...
        try:
            self.csv_dir = self.ui.csv_dir.text().strip()
            self.htm_dir = self.ui.html_dir.text().strip()
            self.set_dir = self.ui.set_dir.text().strip()
            ...
            self.selected_set_files = set_files
            self.logger.info(f"Total Selected files: {self.selected_set_files}")

             # Update CSV list widget
            try:
                self.update_portfolio_list_widget()
                self.update_set_list_widget()
            except Exception as e:
    ...
    def update_portfolio_list_widget(self):
        ...
        try:
            ...
            self.ui.portfolios_created.setSelectionMode(QListWidget.MultiSelection)
            self.logger.debug("Set CSV list widget selection mode to MultiSelection.")

            # Select all items by default
            self.ui.portfolios_created.selectAll()
            self.logger.info("All CSV files selected in the list widget.")

        except Exception as e:
            self.logger.error("Failed to update CSV list widget.", e)

    def update_set_list_widget(self):
        """Update the SET QListWidget with current CSV files."""
        try:
            self.ui.set_files_used.clear()
            self.logger.debug("Cleared SET list widget.")


            for f in self.selected_set_files:
                try:
                    name_without_ext = os.path.splitext(os.path.basename(f))[0]
                    self.ui.set_files_used.addItem(name_without_ext)
                    self.logger.debug(f"Added to SET list widget: {name_without_ext}")
                except Exception as inner_e:
                    self.logger.error(f"Failed to add file to SET list widget: {f}", inner_e)

            # Allow multiple selection
            self.ui.set_files_used.setSelectionMode(QListWidget.MultiSelection)
            self.logger.debug("Set SET list widget selection mode to MultiSelection.")

            # Select all items by default
            self.ui.set_files_used.selectAll()
            self.logger.info("All SET files selected in the list widget.")

        except Exception as e:
            self.logger.error("Failed to update SET list widget.", e)

    def on_build_portfolio_button_clicked(self):
        """
        Trigger CSV compare in background thread.
        """
        items = self.ui.portfolios_created.selectedItems()
        if not items:
        ...
        self.logger.info("Compare started...")
        ...
        def handle_compare_result(result):
            ...
            if not isinstance(result, dict):
                self.logger.info(f"Unexpected result: {result}")
                return

            if "error" in result:
                self.logger.info(f"Compare failed: {result['error']}")
            ...
            if merged_df is None or merged_df.empty:
                self.logger.info("Merged dataframe is empty after merge.")
                self.ui.drawdown_analysis.clear()
                self.ui.portfolio_stats.clear()
                return
            ...
            try:
                ...
                self.logger.info(f"Table update failed: {e}")

            self.logger.info("Compare process completed.")
    ...
    def show_monthly_portfolio_stats(self, merged_df):
        if merged_df is None or merged_df.empty:
            self.logger.info("merged_df is empty.")
            self.ui.portfolio_stats.clear()
            return
        ...
        self.logger.info("Monthly portfolio stats updated (clean & sorted).")
    ...
    def show_drawdown_table(self, merged_df):
        ...
        if df.empty:
            self.logger.info(f"No rows match the filter (SUM_EQUITY ≥ {draw_threshold}).")
            self.ui.drawdown_analysis.clear()
            return
    ...
    def on_show_graph_clicked(self):
        ...
        if not equity_cols:
            self.logger.info("❌ No EQUITY_ columns found.")
            return
        ...
        self.logger.info("Graph displayed successfully.")
